...
\newenvironment{CSLReferences}[2] 
 {\begin{list}{}{
  ...
  \normalsize
  \raggedright % Desactivar justificación en bibliografía
...
\begin{center}
    ...
    {\bfseries \large $faculty$}\\[0.3cm]
    {\bfseries $school$}\\[0.5cm]
    
    \vfill
    
    {\bfseries \large $course$}\\[1cm]
    
    {\bfseries \Huge $assignment$}\\[0.5cm]
    {\large $subtitle$}\\[1cm]
    
    \vfill
    
    {\bfseries \large INTEGRANTES:}\\[0.5cm]
    \setstretch{1.2}
    $if(author)$
        $for(author)$
            {\large $if(author.name)$$author.name$$else$$author$$endif$}\\[0.2cm]
        $endfor$
    $endif$
    
    \vfill
    
    {\bfseries \large DOCENTE:}\\[0.3cm]
    $if(professor)$ {\large $professor$} \\[0.5cm] $endif$
    
    \vfill
    
    {\large LOJA - ECUADOR}\\[0.3cm]
    {\large $submission_date$}
    \vspace*{1.5cm}
\end{center}
\end{titlepage}
\newpage
